...
def volumes_bulk(filenameA, filenameB):
    """Read and print the equilibrium volumes in A^3 and the bulk moduli in GPa for the two phases."""
    dataA = pd.read_csv(filenameA)
    E0A, V0A, B0A, B0_primeA = dataA.iloc[0]

    dataB = pd.read_csv(filenameB)
    E0B, V0B, B0B, B0_primeB = dataB.iloc[0]

    B0A *= (13.6056931229947 * 1.60218e-19) / 1e-30 / 1e9
    B0B *= (13.6056931229947 * 1.60218e-19) / 1e-30 / 1e9

    print(f"A-tin: V = {V0A:.6f} A^3, B = {B0A:.6f} GPa")
    print(f"B-tin: V = {V0B:.6f} A^3, B = {B0B:.6f} GPa")

if __name__ == "__main__":
    phase = Phase("A-params.txt", "B-params.txt")
    phase.common_tangent("A-data.txt", "B-data.txt")
    #volumes_bulk("A-params.txt", "B-params.txt")